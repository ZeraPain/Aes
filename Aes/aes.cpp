...
static unsigned char INV_SBOX[256] = { 0 };
...
bool is_reasonable_data(unsigned char* data)
{
	bool ret = true;
...
int main(int argc, char* argv[])
{
	...
	std::cout << "Text before encryption: ";
	...
	decipher(out, roundKey, out);
	std::cout << "Text after decryption: ";
	for (unsigned int i = 0; i != 4 * 4; ++i)
		std::cout << out[i];
	std::cout << std::endl;

	unsigned char data[] = { 0xbf, 0x3f, 0xb7, 0x7d, 0x93, 0xdd, 0x6c, 0xfd, 0xef, 0xb8, 0x82, 0x2b, 0x82, 0xd0, 0x35, 0x8a };
	unsigned char corruptedKey[] = { 0x81, 0x59, 0x6b, 0xfb, 0x39, 0xc6, 0x2b, 0x71, 0x6e, 0x52, 0xdb, 0x91, 0x81, 0x0, 0x0, 0x0 };
	...
	for (auto i1 = 0; i1 < 256 && !found; ++i1)
	{
		int tmpProgress = i1 / 256.f * 100;
		if (tmpProgress > progress && tmpProgress % 5 == 0)
		{
		...
		corruptedKey[13] = i1;
		for (auto i2 = 0; i2 < 256 && !found; ++i2)
		{
			corruptedKey[14] = i2;
			for (auto i3 = 0; i3 < 256 && !found; ++i3)
			{
				corruptedKey[15] = i3;

				keyExpansion(corruptedKey, roundKey);

				decipher(data, roundKey, out);
				if (is_reasonable_data(out))
				{
					std::cout << std::endl << "Found text: ";
					for (auto i = 0; i != 4 * 4; ++i)
						std::cout << out[i];
					std::cout << std::endl;

					std::cout << "Missing key parts: ";
					for (auto i = 13; i != 4 * 4; ++i)
						std::cout << "0x" << std::hex << (unsigned int)corruptedKey[i] << std::dec << " ";
					std::cout << std::endl;
